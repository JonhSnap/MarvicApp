...
const ArchiveSprint = ({ ArchiveSprint }) => {
  ...
  return (
    <div className="flex flex-col mt-4 border-2 border-blue-500 rounded-lg p-">
      <div className="flex items-center justify-center w-full text-2xl rounded-lg cursor-pointer archive-header bg-violet-500">
        <h2 className="ml-4 textxl">{ArchiveSprint.sprintName}</h2>
        <span>({ArchiveSprint.issues.length} Issues)</span>
      </div>
