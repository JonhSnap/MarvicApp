...
const YourWorkStarred = ({ dataStarred }) => {
  ...
  return (
    <div className="flex items-center px-3 py-1 rounded-lg cursor-pointer hover:bg-slate-200">
      ...
      <div className="flex flex-col ml-2">
        <h2 className="text-base">{dataStarred.name}</h2>
        <span>{dataStarred.key}</span>
      </div>
    </div>
  );
};
