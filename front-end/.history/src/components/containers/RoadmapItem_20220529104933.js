...
const RoadmapItem = ({
  ...
  const handleSelectedEpic = (epicChoose) => {
    // const epicSelectedCopy = [...epicSelected];
    // const result = issueEpics.filter((item) => item.id === epicChoose.id);
  ...
  console.log(epicSelected.filter);
  return (
    <>
      {showEditEpic && (
        <EditEpicPopup
            ...
            <div className="flex items-center justify-between flex-1">
              <div className="flex items-center justify-center">
                <input
                  ...
                  type="checkbox"
                  checked={
                    epicSelected.filter.length === 0
                      ? true
                      : epicSelected.filter.includes(item.id)
                  }
                  className="inline-block w-5 h-5 mx-2 rounded-md bg-slate-300 "
                ></input>
                <span
                  onClick={() => {
