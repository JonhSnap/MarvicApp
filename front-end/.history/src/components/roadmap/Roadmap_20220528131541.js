...
const TooltipContent = ({ onClose, event }) => {
  return (
    <React.Fragment>
      <div className="text-black bg-slate-100 absolute z-50 rounded-lg inline-block ">
        {/* <FontAwesomeIcon icon={faWindowClose} onClick={onClose} /> */}
        <div> To Board {event.title}</div>
      </div>
    </React.Fragment>
  );
};
...
function Event(event) {
  ...
  return (
    <div className="">
      <span onClick={() => setShowTooltip(!showTooltip)}>{event.title}</span>
      {showTooltip && (
        <div>
