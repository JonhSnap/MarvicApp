...
const Comments = ({ commentURL }) => {
  ...
  return (
    <div className="w-[600px] mt-6 h-[650px] border border-1 overflow-x-hidden rounded-lg border-blue-500 p-3 my-auto  mx-auto">
      <div className="comments w-full mx-auto">
        <h3 className="comments-title text-blue-600 select-none">Comments</h3>
        <CommentForm submitLabel="Write" handleSubmit={addComment} />
        <div className="comments-container overflow-y-auto h-[5000px]">
          {comments.length > 0 &&
            comments.map((item) => (
              <Comment
                key={item.id}
