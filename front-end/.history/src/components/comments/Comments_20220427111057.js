...
const Comments = ({ commentURL }) => {
  ...
  const idIssue = "7c2cc804-4aae-4af2-9191-4268fc02edc0";
  const user = useSelector((state) => state.auth.login.currentUser);
  const id_user = user?.id;
  const sendCommentApi = { id_user, idIssue, message };
  const loadComment = async () => {
    await axios
      .get(`${BASE_URL}/api/Comments/issue/${idIssue}`)
      .then((res) => {
  ...
  const addComment = async ({ id_user, id_Issue, message, parentId }) => {
    try {
