...
const Comment = ({
  ...
  const isEditting =
    ...
    activeComment.id === comment.id;
  const replyId = id_ParentComment ? id_ParentComment : reply.id;
  //load comment child
  const [commentChild, setCommentChild] = useState([]);
  useEffect(() => {
    const loadCommentChild = async () => {
      await axios
        .get(`${BASE_URL}/api/Comments/${comment.id}`)
        .then((res) => {
          const testData = res.data.reverse();
          setCommentChild(testData);
        })
