...
const fetchBoard = async (dataGet, dispatch) => {
  try {
    const resp = await axios.get(
      `${BASE_URL}/api/Issue/GetIssueForBoard?idSprint=${dataGet?.idSprint}${
        dataGet?.idEpic ? `&idEpic=${dataGet.idEpic}` : ""
...
const initialValue = {
  ...
  filters: {
    name: "",
    epics: JSON.parse(localStorage.getItem(KEY_FILTER_EPIC)) || [],
    types: [],
  },
};
        ...
        stateCopy.boards.forEach((board) => {
          board.listStage.forEach((stage) => {
            stage.listIssue = stage.listIssue.filter((issue) =>
              issue.summary.toLowerCase().includes(nameFilter.toLowerCase())
            );
          });
        });
      }
