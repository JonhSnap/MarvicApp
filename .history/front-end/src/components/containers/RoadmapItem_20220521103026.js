...
import { faAngleRight, faAnglesDown } from "@fortawesome/free-solid-svg-icons";
...
const RoadmapItem = ({ item, project, epic }) => {
  ...
  return (
    <>
      ...
      <div
        key={v4()}
        className={`parent w-full px-3 py-1 relative flex flex-col font-semibold  shadow-lg rounded-[5px] mb-2 cursor-pointer
        `}
      >
        <div
          className={`w-full px-3 py-1 relative flex flex-col font-semibold  shadow-md rounded-[5px] mb-2 cursor-pointer hover:bg-slate-200`}
        >
